...
const DashboardScreen = () => {
    ...
    try {
      ...
      if (editingEvent) {
        const response = await axios.put(
          `http://10.10.30.156:5000/api/events/${editingEvent._id}`,
          eventPayload,
        ...
        const response = await axios.post(
          'http://10.10.30.156:5000/api/events/create',
          eventPayload,
          { headers: { 'x-auth-token': token } }
        );
        if (response.status === 200) {
  ...
  const handleDeleteEvent = async (eventId: string) => {
    try {
      ...
      const response = await axios.delete(`http://10.10.30.156:5000/api/events/${eventId}`, {
        headers: { 'x-auth-token': token },
      });
