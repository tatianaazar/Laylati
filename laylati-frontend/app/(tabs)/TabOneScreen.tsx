...
const TabOneScreen = () => {
  ...
  useEffect(() => {
    const fetchAllVendors = async () => {
      try {
        const response = await axios.get(`http://10.10.30.156:5000/api/vendors`);
  ...
  const fetchVendors = async (query) => {
    ...
    try {
      const response = await axios.get(`http://10.10.30.156:5000/api/vendors?searchQuery=${query}`);
...
useEffect(() => {
  ...
  const fetchVendorsByCategory = async (category) => {
    try {
      const response = await axios.get(`http://10.10.30.156:5000/api/vendors/${category}`); 
